...
/** 必须成对出现的括号/引号：其中的句末标点不算断句点 */
const PAIRED = { '「': '」', '『': '』', '“': '”', '‘': '’', '（': '）', '(': ')', '《': '》', '【': '】' };
const CLOSERS = new Set(Object.values(PAIRED));

/**
 * 按句末标点切句，**但不从句末标点切在引号/括号内部**。
 *
 * 【坑】最初的实现是 `text.split(/(?<=[。！？!?；;])/)`，它看不见引号：
 *   `林晚说：“必须马上手术。”主任摇了摇头。`
 * 会被切成 `林晚说：“必须马上手术。` 和 `”主任摇了摇头。`
 * ——引号被撕成两半，还多出一个只含 `”` 的碎片 segment。
 * 后果不只是难看：任何用"配对引号"做判据的规则（如判断某句是不是台词）
 * 都会**静默失效**，因为配对引号永远不在同一段里了。
 *
 * 现在的做法是逐字符扫描并维护括号栈：只有栈空时的句末标点才断句。
 * 开括号只有在"后面确实存在对应收尾"时才入栈，避免一个孤立的引号把全文吞成一句。
 */
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    if (stack.length > 0) {
      if (ch === stack[stack.length - 1]) {
        // 关闭括号：**不要 continue**，要落到下面的句末判定。
        // 「林晚说：“必须马上手术。”主任摇了摇头。」里，句末其实发生在收尾引号处；
        // 如果收尾引号这一拍就 continue，断句点会被推到下一个句号，
        // 台词和它后面那句就被粘成一段。
        stack.pop();
      } else {
        if (PAIRED[ch] && src.indexOf(PAIRED[ch], i + 1) >= 0) stack.push(PAIRED[ch]);
        continue; // 括号内部一律不断句
      }
    } else if (PAIRED[ch] && src.indexOf(PAIRED[ch], i + 1) >= 0) {
      // 只在后面确有收尾时入栈；孤立引号按普通字符处理，免得把全文吞成一句
      stack.push(PAIRED[ch]);
      continue;
    }

    if (stack.length === 0 && SENT_END_RE.test(buf)) {
      out.push(buf);
      buf = '';
    }
  }

  if (trim(buf) !== '') out.push(buf);
  return out.map((s) => s).filter((s) => trim(s) !== '');
}
...
export function scanSegments(raw) {
  ...
  for (const para of paras) {
    ...
    const pieces = splitSentences(para.text);
    let localCursor = 0;
