...
export function extractCharacters(segments) {
  ...
  const add = (name, kind, seg, weight, reason) => {
    ...
    if (!bag.has(name)) bag.set(name, { name, kinds: new Set(), evidence: [], score: 0, speech: 0, reasons: new Set(), mentions: 0 });
  ...
  return filtered.map((c, i) => ({
    ...
    name: c.name,
    kinds: [...c.kinds],
    role: pickRole(c, filtered),
    confidence: normScore(Math.min(1, c.score / 20)),
...
function pickRole(c, all) {
  if (c.kinds.has('named') && c.speech >= 2) return 'protagonist?';
