 ...
 * 神话/传说/历史人物表。
 *
 * 【为什么需要】中文姓名抽取靠"姓氏 + 1–2 字"，而神话人物的名字**不带姓氏**：
 * 「精卫」「刑天」「夸父」「女娲」一个都抓不到，于是《读山海经》这类文本会被判成"0 人物"，
 * 后续的骨架、视角、事实锚点全部落空。
 *
 * 只收有共识、且不会与普通词混淆的名字。「沧海」「干戚」这类事象词放在 genre 关键词里，
 * 不放进人物表——它们是物，不是人。
 */
const MYTH_FIGURES = [
  // 《山海经》与上古神话
  '精卫', '刑天', '女娲', '盘古', '夸父', '后羿', '嫦娥', '共工', '蚩尤', '大禹', '神农', '仓颉',
  '炎帝', '黄帝', '羲和', '常羲', '烛龙', '相柳', '西王母', '鲲鹏', '祝融', '玄冥', '句芒', '蓐收',
  '帝俊', '帝江', '英招', '陆吾', '烛阴', '应龙', '旱魃', '颛顼', '少昊', '鲧',
  // 传说与文学人物
  '愚公', '鲁班', '伯牙', '钟子期', '屈原', '项羽', '荆轲', '嵇康', '陶渊明', '曹植',
];
...
export function extractCharacters(segments) {
  ...
  for (const seg of segments) {
    ...
    // 5) 神话/传说人物（名字不带姓氏，姓氏规则抓不到）
    for (const name of MYTH_FIGURES) {
      if (t.includes(name)) add(name, 'mythic', seg, 4.5, '神话/传说人物');
    }
  ...
  // 词典类证据：这些候选来自**封闭词表**（亲属称谓、神话人物、职业称谓），
  // 命中一次就是可靠证据，不该被"出现次数"门槛刷掉。
  // 「母亲一直不肯提起他」里母亲只出现一次，但它确实是个角色。
  const DICTIONARY_REASONS = /亲属称谓|神话\/传说人物|职业称谓|姓氏 \+ 称谓/;
  ...
  const candidates = [...bag.values()]
    .filter((c) => {
      ...
      if ([...c.reasons].some((r) => DICTIONARY_REASONS.test(r))) return true;
...
function pickRole(c) {
  if (c.kinds.has('mythic')) return 'mythic';
...
export function extractNarrative(segments, rawText) {
  ...
  const count = (terms, src = text) => (src.match(new RegExp(terms.join('|'), 'g')) || []).length;

  const first = count(FIRST_PERSON);
  const third = count(THIRD_PERSON);

  // 【坑】「我」出现在**引号内的台词**里时，指的是说话人，不是叙述者。
  // 第三人称小说里遍地都是「我说：」「你别走」，若把它们算进来，
  // 人称判定会被台词带偏。所以先剥掉引号内容，再看叙述层里有没有「我」。
  // 叙述层出现「我」→ 第一人称；这是中文最可靠的判据（比代词比例可靠得多）。
  const narration = text
    .replace(/\u201c[^\u201d]*\u201d/g, '')   // “…”
    .replace(/\u300c[^\u300d]*\u300d/g, '')   // 「…」
    .replace(/\u300e[^\u300f]*\u300f/g, '');  // 『…』
  const firstInNarration = count(FIRST_PERSON, narration);
  const thirdInNarration = count(THIRD_PERSON, narration);

  // 判据只看**叙述层**。不保留"全篇只有我没有他/她就是第一人称"这条兜底：
  // 「林晚说：“我说过，你别走。”」全篇只有「我」没有「他/她」，
  // 但那是台词里的我，叙述其实是第三人称——条兜底会把它判反。
  let person = 'third';
  if (firstInNarration > 0 && firstInNarration >= thirdInNarration) person = 'first';
...
export function extractFacts(segments) {
  ...
  const PATTERNS = [
    ...
    // 年龄一旦写明就不可改写（原文说"十五岁离开家"，演绎不能改成十八岁）。
    // 职业/居所/天气则允许随情节演进，只是同一时点内不得矛盾，故标 false。
    { kind: 'age', re: /([零〇一二三四五六七八九十百0-9]{1,4}\s*(岁|周岁))/, label: '年龄', immutable: true },
