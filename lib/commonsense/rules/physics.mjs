...
function durations(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (m.index > 0 && text[m.index - 1] === '第') continue; // 年份不是时长
...
/** 车辆/重器：PHY-006 必须确认「举的确实是车」，否则「举起杠铃」也会被误报 */
const HEAVY_OBJECT = /(汽车|轿车|面包车|卡车|货车|公交车|客车|坦克|集装箱|装甲车|挖掘机|推土机)/;
...
const VEHICLE = /(车|船|舰|艇|飞机|航班|直升机|飞行|摩托|机车|马|缆车|电梯|无人机|高铁|动车|地铁)/;
...
export const rules = [
  ...
  /* ---------------------------------------------------------------- PHY-004 */
  {
    ...
    check(ctx, hits) {
      ...
      for (const hit of hits) {
        ...
        // 必须真的在水里：屏息以待、屏息凝神都不是「水下憋气」
        if (!/(水|海|河|湖|池|井|潜|泳)/.test(text)) continue;
  ...
  /* ---------------------------------------------------------------- PHY-006 */
  {
    ...
    check(ctx, hits) {
      ...
      for (const hit of hits) {
        ...
        // 必须确认对象就是车辆/重器：只出现「举起」不算（举起杠铃、举起酒杯都是正常的）
        // 动词要容忍「举了起来」「举起了」这些语序，不能只认「举起」
        const lift = '(?:举|抬|扛|搬|抱|推)[起了得来]{0,4}';
        const objIsHeavy = new RegExp(`${lift}[^。；]{0,14}${HEAVY_OBJECT.source}`).test(text)
          || new RegExp(`${HEAVY_OBJECT.source}[^。；]{0,12}${lift}`).test(text);
        if (!objIsHeavy) continue;
        out.push({
          segmentId: hit.id,
          line: hit.line,
          quote: text,
  ...
  /* ---------------------------------------------------------------- PHY-007 */
  {
    ...
    check(ctx, hits) {
      ...
      for (const hit of hits) {
        ...
        // 必须写明「在水里」：站在冰面上、走在冰河上不算冷水浸泡
        if (!/(泡|浸泡|浸在|落水|掉进|掉入|沉入|沉到|游|溺水|水里|水中|挣扎|打捞|捞起|拖上岸|漂浮)/.test(text)) continue;
        const ts = durations(text);
        if (!ts.length) continue;
        const maxHours = Math.max(...ts.map((t) => t.hours));
  ...
  /* ---------------------------------------------------------------- PHY-011 */
  {
    ...
    check(ctx, hits) {
      ...
      for (const hit of hits) {
        const text = hit.text;
        // 「跳上/跳到 N 米」必须紧跟高度语境（高、墙、屋顶…），否则「跳上车，开出三米」也会被算成跳跃高度
        const up = new RegExp(`(?:跳|跃)(?:上|到)[^。；]{0,6}?(${DIST_TOKEN})[^。；]{0,4}?(高|墙|屋顶|房顶|楼|树|平台|台子|栏杆|悬崖|崖|壁)`).exec(text);
        const far = new RegExp(`(?:跳|跃)(?:过|越)[^。；]{0,6}?(${DIST_TOKEN})[^。；]{0,4}?(宽|沟|河|墙|距离|远)`).exec(text);
        const m = up || far;
        if (!m) continue;
  ...
  /* ---------------------------------------------------------------- PHY-012 */
  {
    ...
    check(ctx, hits) {
      ...
      for (const hit of hits) {
        ...
        const lift = /(举起|抬起|抱起|搬起|拎起)/.test(text);
