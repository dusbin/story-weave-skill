...
export const rules = [
  ...
  },
  {
    ...
    check(ctx, hits) {
      ...
      // 「始终没敢拔掉保险销」里的拔出动作并未发生——这类句子不能算命中
      const NOT_PULLED = /(没|没有|未|不敢|没能|来不及|正要|准备|试图)[^。]{0,3}(拔|拉开|拽下|拉掉)/;
      const HOLD = /(握|拿|攥|拎|举)(着|在手里|在手中)/;
      const out = [];
      for (const hit of hits) {
        if (!BOMB.test(hit.text) || !PULL.test(hit.text)) continue;
        if (NOT_PULLED.test(hit.text)) continue;
