...
import { indexSegments, findMatches, segmentsWithAny, segmentsWithAll } from '../text.mjs';
...
// 避免顶层循环依赖：这里按需引入 text.scanSegments 的等价实现
function scanText(text) {
  // 延迟 import 会破坏纯函数的同步性，故内联一份最小切分（与 text.mjs 同规则）
  ...
  for (const para of src.split(/\n[ \t\u3000]*\n+/)) {
    ...
    for (const piece of para.split(/(?<=[。！？!?；;])/)) {
      const t = piece.trim();
      if (!t) { cursor += piece.length; continue; }
      segs.push({
        id: `s${segs.length + 1}`,
        index: segs.length,
        text: t,
...
export function normalizeFinding(raw, rule, ctx) {
  ...
  return {
    ...
    severity: SEVERITY_KEYS.includes(raw.severity) ? raw.severity : (rule.severity ?? 'major'),
