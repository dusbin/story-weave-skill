...
/**
 * term 在该文本中是否被否定（否定词出现在 term 之前 4 个字符内）。
 * 「他不能呼吸」→ 呼吸被否定；「他没想到会下雨」→ 不适用于"下雨"以外，需要调用方自己保证 term 选得对。
 */
export function isNegated(text, term) {
  ...
  while (idx >= 0) {
    ...
    if (NEGATORS.some((n) => before.endsWith(n))) return true;
...
export function parseDuration(text) {
  ...
  //   A「一个半小时」「两个半月」→ n + 0.5
  //   B「半小时」「半个月」       → 0.5
  //   C「三天」「一小时」「十分钟」→ n（系数缺省为 1）
  // 之前用"前面是不是半"来做排除，会把「一个半小时」里那个真正的小时也排掉，
  // 导致 1.5 小时被算成 0.5 小时。
  const re = new RegExp(
  ...
  while ((m = re.exec(t)) !== null) {
    ...
    const numStr = g.aUnit ? g.aNum : g.bUnit ? null : g.cNum;
    ...
    if (unitStr === g.bUnit) {
      n = 0.5; // 分支 B
    } else if (numStr === undefined || numStr === null || numStr === '') {
      n = 1; // 系数缺省：「一小时」「周」前的量词被省略
    } else {
      n = parseNumber(numStr);
      if (n === null) continue; // 「一两个小时」这类概数：判不了就不判
    }
  ...
  return { hours: Math.round(hours * 100) / 100, raw: t, parts };
}
