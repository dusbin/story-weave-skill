 ...
 */
/**
 * 断言式文本的事实锚点：把每一"句"当成原文写死的命题。
 *
 * 适用于一切"由若干独立断言构成、而非叙述事件"的文本：
 * 诗歌韵文、典章诏令、设定片段。它们都没有"死亡/年龄/职业"这类事件词。
 *
 * 【为什么需要】散文靠"死亡/年龄/职业/天气"这类**事件型**事实做锚点，
 * 而诗里没有这些——「精卫衔微木，将以填沧海」不含任何事件模式词，
 * 于是事实锚点为空，硬约束表里只剩通用的模式不变量，
 * 写作指令完全没有"不得改写什么"的具体依据。
 *
 * 诗的每一句本身就是被写死的意象与命题，因此逐句立锚，
 * 并明确标注为"原文诗句"，让报告与写作指令都能指出具体是哪一句。
 */
export function extractVerseFacts(segments, { label = '原文诗句' } = {}) {
  const out = [];
  let n = 0;
  for (const seg of segments) {
    // 以"顿"为单位：一句里可能含两个五言
    const clauses = String(seg.text).split(/[，、,]/).map((x) => x.trim()).filter(Boolean);
    for (const clause of clauses) {
      const clean = clause.replace(/[。！？!?…\s]+$/, '');
      // 下限取 3 而不是 4：条款往往很短，而每一个短句都可能是硬约束。
      // 「八月成」「名曰酎」都只有三字，却是酎酒制度的关键命题——
      // 早先按 4 字下限把它们静默丢掉，正文就再也受不到它们的约束。
...
export function analyzeText({ raw, opts = {} } = {}) {
  ...
  // 文本类型要尽早算出来：诗歌锚定策略依赖它，模式评分（completeness）也依赖它
  const textTypeEarly = classifyTextType(segments, stats, genre, narrative);
  ...
  // 诗歌/韵文：事件型锚点通常为空，改为逐句立锚（见 extractVerseFacts 的说明）
  // 断言式文本（诗、典章、设定）：通篇由独立命题构成，没有事件型事实词，
  // 于是事件抽取结果为空。这类文本的每一句本身就是被写死的命题，逐句立锚。
  ...
  const attrLabel = ASSERTION_TYPES[textTypeEarly?.primary]
    ?? (textTypeEarly?.scores?.classical_poem >= 0.7 ? '原文诗句' : null);
  const factsFinal = (attrLabel && facts.length === 0)
    ? extractVerseFacts(segments, { label: attrLabel })
    : facts;

  // 先组一个"半成品"分析对象，供 gaps 与 modeFit 使用
  const partial = {
    source: { chars: stats.chars, fingerprint: fingerprint(text) },
    genre, standard, narrative, elements: { characters, timeline, places }, world, conflicts, facts: factsFinal,
    meta: { firstSegment: segments[0]?.text ?? '', lastSegment: segments[segments.length - 1]?.text ?? '' },
  };

  const gaps = detectGaps(segments, partial);
  partial.gaps = gaps;
  partial.textType = textTypeEarly;
...
const TEXT_TYPES = [
  ...
  // 典章诏令：以「制/诏/敕/谕」等文种名开头，或通篇是制度条款
  { key: 'document', label: '典章/诏令', re: /^\s*(制|诏|敕|谕|旨|令|律|疏|表|奏)\s*[：:]/, hint: '制度条款/公文' },
...
export function classifyTextType(segments, stats, genre, narrative) {
  ...
  for (const t of TEXT_TYPES) {
    ...
    if (t.key === 'outline') {
      ...
      // 文种名（制/诏/敕…）开头的公文；与"某人："式对话要分清
      s = /^\s*(制|诏|敕|谕|旨|律)\s*[：:]/.test(text) ? 0.85 : 0;
    } else if (t.key === 'dialogue_log') {
      // 【坑】「制：」是文种名，不是说话人。早先只看"开头若干字 + 冒号"，
      // 于是诏令被整段判成对话记录（0.75），还要给它找个"台词"。
      const isDocPrefix = /^\s*(制|诏|敕|谕|旨|令|律|疏|表|奏)\s*[：:]/.test(text);
      const dl = lines.filter((l) => /^[^：:\n]{1,8}[：:]/.test(l.trim()))
        .filter((l) => !/^\s*(制|诏|敕|谕|旨|令|律|疏|表|奏)\s*[：:]/.test(l.trim())).length;
      s = !isDocPrefix && lines.length && dl / lines.length > 0.4 ? 0.75 : 0;
