 ...
 */
export function extractVerseFacts(segments) {
  const out = [];
  let n = 0;
...
export function analyzeText({ raw, opts = {} } = {}) {
  ...
  const isPoetic = ['classical_poem', 'lyric'].includes(textTypeEarly?.primary) || textTypeEarly?.scores?.classical_poem >= 0.7;
  const factsFinal = (isPoetic && facts.length === 0) ? extractVerseFacts(segments) : facts;
