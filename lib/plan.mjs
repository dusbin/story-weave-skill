...
function buildLogline(an, modeKey, protagonist, place, scaleKey) {
  const conflict = an.conflicts?.[0]?.type ?? null;
  ...
  // 没识别出冲突时不要写"核心冲突：未明确的冲突"——那是把占位符当内容输出，
  // 会原样进到正文页眉与交付文档里（用一首古诗跑前传时就是这么露出来的）。
  const conflictPart = conflict ? `核心冲突：${conflict}。` : '';
  return `${verbs[modeKey] ?? '演绎原文'}。${conflictPart}篇幅：${SCALE[scaleKey]?.label ?? scaleKey}。`;
}
