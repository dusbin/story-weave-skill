...
export const EXPAND = {
  ...
  skeleton(ctx) {
    ...
    return [
      { title: '落地：把原文的第一句展开成场景', purpose: '让读者"站在现场"', guidance: `从原文起点写起，补足时间、地点、天气、身体感受。若原文首句是「${ctx.firstQuote || '原文首句'}」，就以它为锚，向前后各展开，但不得改变其含义。` },
      ...
      { title: '收束回到原文的结尾状态', purpose: '闭合因果', guidance: `演绎必须落在原文已有的终点上${ctx.lastQuote ? `（「${ctx.lastQuote || '原文结尾'}」）` : ''}。最后一段收在原文那个画面上，读者能看出"原来是这么走到这一步的"。` },
    ];
  },
...
export const CONTINUE = {
  ...
  skeleton(ctx) {
    return [
      { title: '紧接原文最后一句', purpose: '无缝承接', guidance: `从「${ctx.lastQuote || '原文结尾'}」之后立刻开始，不要重新交代背景。第一段就要让读者感到故事在继续。` },
      { title: '直接后果', purpose: '兑现原文埋下的因果', guidance: '原文结尾那件事的即时后果（身体上的、关系上的、现实层面的）。' },
      { title: '人物反应与代价', purpose: '写人', guidance: `${ctx.protagonist || '主角'}对后果的反应必须符合原文已表现的性格；反应之后要付出代价。` },
...
export const PREQUEL = {
  ...
  skeleton(ctx) {
    return [
      ...
      { title: '第一次选择', purpose: '启动因果', guidance: `${ctx.protagonist || '主角'}做了一个当时看来合理的选择，这个选择决定了后面的一切。` },
      ...
      { title: '接上原文开头', purpose: '闭环', guidance: `最后一节必须严丝合缝地接上原文第一句（「${ctx.firstQuote || '原文首句'}」）。写完后自我检查：读者读完前传再读原文，会觉得"原来如此"。` },
    ];
  },
 ...
 */
// 注意：这里的 ctx 在 `sw modes` 预览时是空的（没有分析结果）。
  // 骨架里的 ctx.firstQuote / ctx.lastQuote 必须给兜底值，
  // 否则预览会打印出「从「undefined」之后立刻开始」这种字面 undefined。
export function buildBeats(mode, rawCtx = {}) {
  // 统一补齐上下文默认值。注意：后续拼接一律用 `||` 而非 `??`，
  // 因为这里的空字符串也应当回退到描述性兜底文案。`sw modes` 预览时没有分析结果，
  // ctx 是空的；不给兜底就会打出「undefined的处境与想要的东西」这种标题。
  const ctx = {
    protagonist: rawCtx.protagonist ?? '主角',
    place: rawCtx.place ?? '',
    firstQuote: rawCtx.firstQuote ?? '',
    lastQuote: rawCtx.lastQuote ?? '',
    spinoffCharacter: rawCtx.spinoffCharacter ?? '',
    adaptAxis: rawCtx.adaptAxis ?? '',
    branchVariable: rawCtx.branchVariable ?? '',
    sections: rawCtx.sections,
  };
