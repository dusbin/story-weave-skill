...
export const EXPAND = {
  ...
  skeleton(ctx) {
    ...
    return [
      ...
      { title: '临界时刻', purpose: '情绪与情节的峰值', guidance: '角色必须在两个都不好的选项里选一个。写清他为什么选这个。' },
...
export const CONTINUE = {
  ...
  skeleton(ctx) {
    return [
      ...
      { title: '选择与后果', purpose: '兑现主题', guidance: '角色做出选择并承担后果——不一定要好结局，但必须是他自己的选择导致。' },
...
export const PREQUEL = {
  ...
  invariants: [
    ...COMMON_INVARIANTS,
    '前传的终点必须能推出原文的起点：原文说"哥哥十五岁离开家"，前传就要写到十五岁离开的那一刻。',
  ...
  skeleton(ctx) {
    return [
      ...
      { title: '无法回头的一刻', purpose: '不可逆', guidance: '发生一件事，使事情再也无法回到从前。这一步必须与原文的某个结果对应。' },
...
export const ADAPT = {
  ...
  skeleton(ctx) {
    return [
      ...
      { title: '代价与选择', purpose: '保留内核重量', guidance: '原文的选择及其代价必须保留，这是内核的本体。' },
...
export const SPINOFF = {
  ...
  skeleton(ctx) {
    return [
      ...
      { title: '一个小冲突', purpose: '避免流水账', guidance: '冲突要小但真实（面子、习惯、说不出口的话），不要动用主线级别的大事件。' },
...
export const WHATIF = {
  ...
  skeleton(ctx) {
    return [
      ...
      { title: '分叉点重演：变量取新值', purpose: '唯一变量生效', guidance: '回到分叉点，只改那一个变量。**这一步必须让读者清楚看到"只改了这个"**。' },
...
function fitBeatCount(base, target, ctx, mode) {
  ...
  if (base.length > target) {
    ...
    const keep = Math.max(0, target - 2);
    const picked = [];
    for (let i = 0; i < keep; i++) {
      const idx = Math.floor((i * middle.length) / keep);
      picked.push(middle[Math.min(idx, middle.length - 1)]);
    }
    return [head, ...picked, tail].map(clone);
  }
...
function clone(b) {
  return { title: b.title, purpose: b.purpose, guidance: b.guidance, branch: b.branch };
}
