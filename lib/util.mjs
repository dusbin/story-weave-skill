...
/** 只有数字字、没有任何单位（十百千万亿） */
const DIGIT_CHARS_ONLY = /^[零〇一壹二贰两三叁四肆五伍六陆七柒八捌九玖]+$/;
...
function parseCnInt(s) {
  ...
  // 全是数字字、没有单位：要么是逐位读法（含「零」，如 一零二 = 102），
  // 要么是汉语约数（一两、三五）——后者判不了，必须返回 null。
  if (DIGIT_CHARS_ONLY.test(s)) {
    if (s.length === 1) return CN_DIGIT[s];
    if (!literal) return null;
    let n = 0;
    for (const ch of s) n = n * 10 + CN_DIGIT[ch];
    return n;
  }

  // 【坑】连续数字字必须被单位隔开，否则不是合法数量词。
  // 「一两」不是 2、「三五」不是 5——它们是汉语的约数表达（一两个、三五天）。
  // 旧实现取最后一个数字字，把「一两个小时」静默解析成恰好 2 小时，
  // 于是"约数"被当成"确定值"参与量级判定。
  const runs = s.match(/[零〇一壹二贰两三叁四肆五伍六陆七柒八捌九玖]{2,}/g) ?? [];
  for (const run of runs) {
    if (!/[零〇]/.test(run)) return null;
  }
  ...
  if (pending !== null) {
    // 末尾裸数字：按**紧邻的那个单位**的下一档解读（「一千二」=1200、「二十五」=25）。
    // 【坑】必须用"最近一次出现的单位"，不能用"最大的单位"：
    // 「九千九百九十九」末尾的九紧跟「十」，按 10/10=1 解读得 9999；
    // 若按最大单位「千」解读会算成 10890——静默错 891，而字数/时长判定全都会跟着错。
    if (!literal && trailingAfterUnit && lastUnit > 0) total += pending * (lastUnit / 10);
    else total += pending;
  }
  return total;
}
